...
server <- function(input, output) {
  ...
  output$YearOutput <- renderUI({
    selectInput("YearInput", "Year", sort(unique(data$Year)), selected = "AL")  })
  
  output$diseaseOutput <- renderUI({
    selectInput("diseaseInput", "Disease/Condition", sort(unique(data$ICD.Chapter)), selected = "Certain infectious and parasitic diseases")})
